...
export function getBlockHeader(block: IBaseBlock) {
  ...
  console.log({
    ...
    n: ethUtils.bufferToHex(n),
    ts: ethUtils.bufferToHex(ts),
    txRoot: ethUtils.bufferToHex(txRoot),
    receiptsRoot: ethUtils.bufferToHex(receiptsRoot),
    hash: ethUtils.bufferToHex(
      ethUtils.keccak256(Buffer.concat([n, ts, txRoot, receiptsRoot]))
    )
  })
...
export async function buildPayloadForExit(
  ...
  //   console.log(
  //     blockProof,
  //     tree.leaves.map(e => e.toString('hex')),
  //     tree.layers,
  //     getBlockHeader(
  //       (await ethers.provider.getBlock(block.number)) as unknown as IBaseBlock
  //     ).toString('hex'),
  //     block
  //   )
  ...
  // console.log({ root: ethUtils.bufferToHex(tree.getRoot()), proof: blockProof })
  // console.log('verifyProof', tree.verify(fullBlock, 1, tree.getRoot(), tree.getProof(fullBlock)))

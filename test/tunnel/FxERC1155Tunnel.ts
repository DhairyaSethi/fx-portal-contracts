...
describe('FxERC1155Tunnel', () => {
  ...
  it.only('fxChild, withdraw success syncing with root chain', async () => {
    let childTokenMap = await fxERC1155ChildTunnel.rootToChildToken(
      fxERC1155.address
    )
    let rootTokenMap = await fxERC1155RootTunnel.rootToChildTokens(
    ...
    const checkpointManager: MockCheckpointManager =
      new MockCheckpointManager__factory(wallet).attach(
        await fxERC1155RootTunnel.checkpointManager()
      )

    // console.log({burnProof, root, managerAddr: await fxERC1155RootTunnel.checkpointManager()})

    // console.log('here',await checkpointManager.resolvedAddress)
    // await checkpointManager.submitCheckpoint(500, {
    //   createdAt: 0,proposer: '',
    //   start: withdrawTx.blockNumber - 1,
    //   end: withdrawTx.blockNumber,
    //   root
    // })
